...
void RunAction::BeginOfRunAction(const G4Run*)
{
  ...
  analysisManager->CreateNtuple("Energy", "Secondary particles");
  analysisManager->CreateNtupleIColumn("volumeID");
...
void RunAction::EndOfRunAction(const G4Run* run)
{
  ...
  G4double dose = 0., rmsDose = 0.;
  G4double mass = 0.;
  if (detConstruction->GetScoringVolume()) {
    mass = detConstruction->GetScoringVolume()->GetMass();
    dose = edep / mass;
    rmsDose = rms / mass;
  }
